...
<div class="content-header"><h2 class="content-title">Add Downtime Matrix Rule</h2></div>
        ...
        <div class="table-wrapper" style="max-width: 600px;"><form method="POST" action="{{ route('downtime-matrix.store') }}" class="ajax-form" style="padding: 1.5rem;">@csrf<div class="form-group @error('origin_facility_id') has-error @enderror"><label for="origin_facility_id">Origin Facility *</label><select id="origin_facility_id" name="origin_facility_id" required>@foreach($facilities as $f)<option value="{{ $f->facility_id }}">{{ $f->facility_name }}</option>@endforeach</select>@error('origin_facility_id')<div class="error-message">{{ $message }}</div>@enderror</div>

        <div class="form-group @error('destination_facility_id') has-error @enderror"><label for="destination_facility_id">Destination Facility *</label><select id="destination_facility_id" name="destination_facility_id" required>@foreach($facilities as $f)<option value="{{ $f->facility_id }}">{{ $f->facility_name }}</option>@endforeach</select>@error('destination_facility_id')<div class="error-message">{{ $message }}</div>@enderror</div>
