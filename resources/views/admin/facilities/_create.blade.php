<div class="content-header">
    <h2 class="content-title">Add Facility</h2>
</div>

<div class="table-wrapper" style="max-width: 600px;">
    <form method="POST" action="{{ route('facilities.store') }}" class="ajax-form" style="padding: 1.5rem;">
        @csrf

        <div class="form-group @error('facility_code') has-error @enderror">
            <label for="facility_code">Facility Code *</label>
            <input type="text" id="facility_code" name="facility_code" value="{{ old('facility_code') }}" required>
            @error('facility_code')
        ...
        <div class="form-group @error('facility_type_id') has-error @enderror">
            ...
            <select id="facility_type_id" name="facility_type_id" required>
                ...
                @foreach($facility_types as $type)
                    <option value="{{ $type->facility_type_id }}" {{ old('facility_type_id') == $type->facility_type_id ? 'selected' : '' }}>{{ $type->facility_type_name }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group @error('facility_category_id') has-error @enderror">
            ...
            <select id="facility_category_id" name="facility_category_id" required>
                ...
                @foreach($facility_categories as $category)
                    <option value="{{ $category->facility_category_id }}" {{ old('facility_category_id') == $category->facility_category_id ? 'selected' : '' }}>{{ $category->facility_category_name }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            <label>
                <input type="hidden" name="is_rtl" value="0"><input type="checkbox" name="is_rtl" value="1" {{ old('is_rtl') ? 'checked' : '' }}>
                RTL Farm
            </label>
        </div>
        ...
        <div class="form-group">
            <label>
                <input type="hidden" name="is_gs" value="0"><input type="checkbox" name="is_gs" value="1" {{ old('is_gs') ? 'checked' : '' }}>
                Gatesale Enabled
            </label>
        </div>
