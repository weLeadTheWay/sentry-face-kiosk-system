...
This file is the primary system/business context for Claude sessions working on this repository. It is derived from the actual current implementation (read on 2026-08-26; a **Multi-Pose Face Enrollment upgrade** (Visitor Registration + Kiosk Self-Service) was added 2026-09-03 across four shipped phases — Phase A: `face_profile_embedding` child table, `FaceMatchingService::match()` (min-distance-across-poses scoring + ambiguity detection), config-driven threshold, all backward-compatible with existing single-embedding `face_profile` rows, zero user-facing behavior change; Phase B: `VisitorRegistrationService`/`KioskController` write paths switched to a generic `poses` payload (any subset of FRONT/LEFT/RIGHT, never fabricated) and `KioskController::recognize()` switched to `match()` with fail-safe-on-ambiguous handling; Phase C: the shared guided-capture frontend module `public/js/face-enrollment.js` (landmark-driven pose/position guidance, auto-capture) wired into both `capture.blade.php` and `kiosk/show.blade.php`'s Gatesale/Truck registration; Phase C's own manual testing then found the LEFT/RIGHT yaw-direction mapping inverted from the user's physical perspective — fixed same-day (see `yawMatchesTarget()` in `public/js/face-enrollment.js`); Phase D (same-day, 2026-09-03) was a full audit/cleanup/verification pass across all of Phases A–C — confirmed the architecture, backward compatibility, and separation of concerns all hold, removed one genuinely-unused export (`FaceEnrollment.POSE_INSTRUCTIONS`) and two stale comments, found (but deliberately did not touch) a minor duplication where both `capture.blade.php` and `kiosk/show.blade.php` independently define their own pose-instruction display strings rather than sharing one source — **Phase E (legacy-profile backfill command, shared test factory/trait, threshold/margin recalibration) still has not been done**; see the "Face Matching", "Visitor Registration", and "Kiosk Self-Service" module sections for full detail; the "Facility Master Data" module and its tables were added 2026-08-27 in two same-day phases — Phase 1 built the tables as dormant/parallel data, Phase 2 cut `visitor_request`/`kiosk_device`/Visitor Sync over to depend on them for real — see that module's section for exactly what is and isn't wired in; the "Downtime Matrix PDF Import" module — Phase 1, parse/validate/preview only, no production-table writes — was added 2026-08-27; its Phase 2 (added 2026-08-28) added a real "Save to Production" mapping step that writes verified imports' eligible rows into `downtime_matrix`/`downtime_stationary` (deactivating, never deleting, whatever was previously active) — see that module's section for the full mapping rules; on 2026-08-28, every admin listing under `Admin Management` (Facilities, Facility Aliases, Kiosk Devices, Identity Types, Employee Types, Downtime Matrix, Downtime Stationary, the Downtime Matrix Import *list*, Roles, Users, Audit Logs) was migrated — through two superseded intermediate approaches, both now fully removed — to a real jQuery DataTables.js implementation: an empty Data Table Shell on page load, filter controls that never fire a request on their own, and a Filter-button click that's the sole trigger for the first (and only the first) automatic request to a dedicated per-module `/data` JSON endpoint using DataTables' server-side processing protocol; see the "Admin Management" module's "Admin Data Table Architecture" subsection. Later the same day this was extended to the Downtime Matrix Import *Preview/show* page too — its three category tabs (Farm-to-Farm/Stationary/Others) are each their own filter-gated Data Table sharing one `rows-data` endpoint, filtered by `rule_type`; see the "Downtime Matrix PDF Import" module. On 2026-09-03, after the Multi-Pose Face Enrollment upgrade above, a separate same-day change added `facility_list.is_gs` — a plain, non-nullable boolean (default `false`) gating whether a facility is eligible for the Kiosk Self-Service **Gatesale** process specifically (Truck, which shares nearly all of the same code, was deliberately left ungated, since it has no flag of its own and gating it too would have silently broken Truck at every existing facility the moment this column's safe default landed); enforced server-side at all three of Gatesale's direct write/route-in entry points (`handleSelfServiceRecognition()`, `createGatesaleVisit()`, `gatesaleRegisterIdentity()`) — this is the actual, load-bearing check — and only blocks *starting* a new Gatesale visit/identity, never a visit already in progress if the flag is later turned off; the admin Facility create/edit forms got a matching "Gatesale Enabled" checkbox (Phase 3's CRUD, extended) since every existing facility starts disabled and an admin must explicitly opt each one in. The same day, after live testing showed a visitor could fill out the whole registration form only to be rejected at Save, a follow-up change also disabled the Gatesale `<option>` in the kiosk's registration-type dropdown server-side (`kiosk/show.blade.php`, driven by `$kiosk->facility?->is_gs`) and bumped up the main recognition/enrollment webcam and panel sizes for readability (a plain CSS pass, no behavior change) — see the "Kiosk Self-Service" and "Facility Master Data" module sections for the full design. Later the same day (2026-09-03), a **Facility Configuration** admin module was added — a single centralized checkbox-grid screen (`admin/facility-configuration`, reusing the `facilities.manage` permission) listing every facility with Gatesale/Break Enabled/Truck toggle columns, so an admin no longer has to open each facility's own edit form individually to flip these; it added two more `facility_list` columns alongside `is_gs`: `is_break_enabled` (boolean, **defaults `true`** — the only one of the three whose default preserves existing behavior, since unlimited IN/OUT breaks during one visitation was already the universal pre-existing behavior everywhere) and `is_truck` (boolean, defaults `false`, mirroring `is_gs`'s polarity). `is_break_enabled` is enforced in `VisitorKioskService::processEntry()` — a `temporary_exit` action is rejected server-side whenever the visit's facility has it `false`, and the `/kiosk/{kiosk}/recognize` response now carries a top-level `break_enabled` field the kiosk frontend uses to hide the "Go Outside" button (never render it) while still always offering "Leave Farm", producing a strict one-IN/one-OUT visitation; the restriction is evaluated per **visitation+facility** (via `visitor_request.facility_id`, fixed at creation), never globally per person, and an already-started break (begun while enabled) is never retroactively invalidated by a later flag flip — only *starting a new* break is gated. This rule applies uniformly to every visitor type that shares `processEntry()` (Visitor-with-Approval, Gatesale, Truck). Truck enforcement (`is_truck`) was added as a second, later same-day change, explicitly mirroring `is_gs`'s design point-for-point — `isTruckEligibleFacility()`/`truckNotEligibleResponse()` alongside the existing `isGatesaleEligibleFacility()`/`gatesaleNotEligibleResponse()`, checked at the same three direct entry points (`handleSelfServiceRecognition()`, `createGatesaleVisit()`, `gatesaleRegisterIdentity()`), same null-safe fail-closed behavior, same "only gates starting a new visit/identity, never disrupts one in progress" rule, and the kiosk's `#gatesale-reg-type` select now disables the Truck `<option>` the same way it already disabled Gatesale's — see the "Kiosk Self-Service" and "Facility Master Data" module sections for the full detail. Later the same day (2026-09-03), a **guided-capture visual overlay** was added as a purely presentational companion to `face-enrollment.js` — two new shared files, `public/css/face-guide.css` and `public/js/face-guide-ui.js` (`FaceGuideUI.create(containerEl)` → `{update, reset, destroy}`), rendering a Face-ID-style radial tick-mark ring, a scanning sweep line, animated LEFT/RIGHT turn-direction hints, and a checkmark badge, all driven purely from the exact same `onStateChange(state, meta)` pairs `FaceEnrollment.start()` already emits — it adds no new detection/capture logic and does not change the `poses` payload, the enrollment/dedup contract, or either host page's existing status-text/pose-dot handling, which both remain wired up unchanged alongside it. Wired into both `capture.blade.php` (created once at page load, over `.video-container`) and `kiosk/show.blade.php`'s Gatesale/Truck registration screen (created lazily on first "Registration" tap, over `#enrollment-webcam`'s existing wrapper, reset on cancel/idle-timeout) — see "Visitor Registration" and "Kiosk Self-Service" for the wiring detail. Where the implementation was ambiguous or unverifiable from static reading, this is marked `NEEDS VERIFICATION` or `UNKNOWN` rather than guessed.
...
Visitor Registration
 ...
 ├── public/css/face-guide.css + public/js/face-guide-ui.js (added 2026-09-03, shared with Kiosk
 │      Self-Service the same way - purely visual ring/scan-line/hint/badge overlay driven by
 │      face-enrollment.js's own onStateChange(state, meta), no detection/capture logic of its own)
...
| JS/CSS | `public/js/face-guide-ui.js`, `public/css/face-guide.css` (new 2026-09-03) | Purely visual ring/scan-line/hint/badge overlay, driven by `face-enrollment.js`'s own `onStateChange` — no detection/capture logic; shared with Kiosk Self-Service's registration screen the same way `face-enrollment.js` itself is |
| Views | `resources/views/visitor/{register,capture,search,success}.blade.php` | Public UI (face-api.js/jsQR loaded client-side); `capture.blade.php` rewritten 2026-09-03 to drive `FaceEnrollment` instead of a manual single-shot capture button, and (later the same day) to also instantiate `FaceGuideUI` over `.video-container` |
...
**Important Notes for Future Changes:** Do not let Option A/B create a `face_profile` before confirming there's no existing match — the current ordering (match-check first, create second) is what prevents duplicate face rows for the same person; this is unchanged by the multi-pose upgrade, just now checked against the FRONT pose specifically. Do not change the QR payload to anything other than the raw `visitor_id` without also updating the kiosk's QR decode expectations. Do not have `completeFaceRegistrationOptionA()` require exactly 3 pose keys — the generic "however many were captured" contract is deliberate (see Business Rules) and is what Phase E's eventual work (or any future capture-flow variant) can keep relying on without another backend change. If you ever change `face-enrollment.js`'s tunable constants, do it in that one file (`FaceEnrollment.config`) — both this page and Kiosk Self-Service's registration screen inherit the change automatically. **If you ever touch `computeYawOffset()`/`yawMatchesTarget()`, remember face-api.js detects against the RAW camera frame, not the CSS-mirrored on-screen preview** — a same-day Phase C manual-testing fix found the LEFT/RIGHT sign mapping inverted from the user's physical perspective for exactly this reason (the raw frame is effectively "facing the user," so their physical right lands on the frame's left side, same convention as 68-point landmark index 0); see `yawMatchesTarget()`'s docblock in the file for the full geometry before changing either function again. Similarly, any change to the visual scan overlay (ring color/timing, scan-line animation, turn hints, checkmark badge) belongs in `public/js/face-guide-ui.js`/`public/css/face-guide.css` (added 2026-09-03) — not forked inline into this page or Kiosk Self-Service's — since both pages instantiate the same `FaceGuideUI` module; it is presentation-only and must stay that way (no detection/capture logic, no new fields on the `poses` payload).
...
- **Break eligibility is a per-facility flag, `facility_list.is_break_enabled` (added 2026-09-03, defaults `true`):** `VisitorKioskService::processEntry()` rejects a `temporary_exit` action with `{success:false}` whenever the visit's own facility (`$visitorRequest->facility`) has this `false` — a break-disabled visitation is strictly `first_entry` → `final_exit`, with no intermediate `Outside` state ever reachable. This is evaluated per **visitation+facility** (the visit's fixed `facility_id`), never globally per person — the same person can have a break-disabled visit at one facility and a break-enabled one at another, concurrently or on different dates. `return`/`final_exit` are never gated by this flag — only *starting* a new `temporary_exit` is; a break already in progress (started while the facility was break-enabled) is never retroactively invalidated if an admin later flips the flag off mid-visit. `buildRecognitionResponse()` also returns a top-level `break_enabled` field (from `$kiosk->facility?->is_break_enabled`) purely so the kiosk frontend can hide the "Go Outside" button — this is a UX convenience only; the `processEntry()` guard above is the actual, load-bearing enforcement and is checked independent of what the client sends. Applies identically to Gatesale/Truck since they share `processEntry()`. See the "Facility Master Data" module and the "Facility Configuration" admin module (under Admin Management) for where this flag is set.
...
### Module: `Kiosk Self-Service` (Gatesale & Truck) — Guided Multi-Pose Capture added 2026-09-03 (Phase C); Gatesale facility-eligibility flag added 2026-09-03
...
handleSelfServiceRecognition()
   ...
   │      [checked BEFORE the is_gs/is_truck gates below - a visit already in progress is never
   │       disrupted by a facility's flag being turned off mid-visit]
   ├── an ACTIVE request exists at a DIFFERENT farm                    → 403 gatesale_active_elsewhere (must finish that visit first)
   ├── no ACTIVE request anywhere, visitor_type = Gatesale, facility_list.is_gs = false
   │                                                                    → 403 gatesale_not_available
   ├── no ACTIVE request anywhere, visitor_type = Truck, facility_list.is_truck = false
   │                                                                    → 403 truck_not_available (added 2026-09-03, mirrors is_gs exactly)
   └── no ACTIVE request anywhere (otherwise)                          → gatesale_confirm_identity ("Is this you?")
                                                                              ↓
...
FaceMatchingService::findMatch(poses.FRONT.descriptor)  [unscoped, re-checked to avoid a duplicate on
   ...
   │        this independently re-checks is_gs against the MATCHED directory's own visitor_type, not the
   │        requested visitor_type, so a Gatesale registration attempt that actually matches an existing
   │        Truck directory is correctly routed through unaffected by is_gs)
   ├── match found + NOT Gatesale/Truck              → 422 identity_not_supported (no conversion, no duplicate profile)
   ├── no match, requested visitor_type = Gatesale, facility_list.is_gs = false → 403 gatesale_not_available (new-identity
   │        registration path - checked here BEFORE the transaction opens, so no row is ever written)
...
Kiosk Self-Service
 ...
 ├── public/css/face-guide.css + public/js/face-guide-ui.js (new 2026-09-03, same day, later — shared
 │      the same way as face-enrollment.js itself; purely visual, driven by its onStateChange)
...
- **Gatesale facility eligibility (`facility_list.is_gs`, added 2026-09-03).** A facility must have `is_gs = 1` before its kiosk will allow the Gatesale self-service process. **Originally Truck was deliberately left ungated by this flag** (it had no flag of its own, and gating it too would have silently broken every existing Truck deployment the moment `is_gs`'s `false` default landed) — a later same-day change (see the `is_truck` bullet below) gave Truck its own independent, analogous gate instead of reusing `is_gs`; the two flags are fully independent (a facility can allow one, both, or neither). Enforced independently, server-side, at every direct entry point (not only via the UI's recognition-time routing) via the shared `KioskController::isGatesaleEligibleFacility(KioskDevice $kiosk): bool` helper (`(bool) $kiosk->facility?->is_gs` — null-safe, so a kiosk whose facility somehow fails to resolve is treated as **not** eligible, never as an error or an open default):
  - `handleSelfServiceRecognition()` — checked only in the "no ACTIVE request anywhere" branch, for a directory whose own `visitor_type` is Gatesale. An already-ACTIVE request (created while the facility was eligible) is resumed unconditionally, never retroactively blocked by a later flag flip.
  - `createGatesaleVisit()` — checked only in the "no existing active request, about to create a brand-new one" branch (after the different-farm re-check, before the Host/Origin/Purpose validation), for a Gatesale directory. This is the actual defense against a client calling `gatesale/create-visit` directly with a known `directory_id`, bypassing `recognize()` entirely.
  - `gatesaleRegisterIdentity()` — checked only in the "no face match, registering a brand-new identity" branch, when the **requested** `visitor_type` is Gatesale, before the transaction opens (so a rejected registration never writes a partial row). The "matched an existing directory" branch is not separately checked here — it re-routes through `handleSelfServiceRecognition()`, which checks eligibility against that directory's *actual* type.
  - `gatesaleUpdateDetails()` is deliberately **not** gated — it only edits a self-service directory's own contact fields and never creates a request/session, so blocking it would add UX friction with no corresponding security benefit (visit creation and new-identity registration are the two places that actually matter, and both are covered above).
  - Failure shape: `403 {success: false, type: 'gatesale_not_available', message: 'Gatesale self-service is not available at this facility.'}` from `handleSelfServiceRecognition()`/`gatesaleRegisterIdentity()`; a plain `abort(403, '...')` from inside `createGatesaleVisit()`'s transaction closure (same pattern already used there for the cross-farm check, since that closure's return type is `VisitorRequest`, not a `JsonResponse`).
  - Every existing facility defaults to `is_gs = false` (the migration's column default) — there is no other source of truth to infer historical Gatesale usage from, so this is a genuine "everyone starts disabled, an admin must explicitly re-enable per facility via `admin/facilities`" cutover, not a no-op default. See "Facility Master Data"'s Known Constraints for this operational implication.
  - **Frontend enforcement (added 2026-09-03, same day):** `KioskController::show()` now eager-loads `$kiosk->load('facility')` and passes it to the view; `kiosk/show.blade.php` renders the "Gatesale" `<option>` in the new-registration `#gatesale-reg-type` select with the `disabled` attribute (plus an inline "(not available at this facility)" label) whenever `$kiosk->facility?->is_gs` is falsy, and a matching JS `const gatesaleEnabled` flag makes `onGatesaleEnrollmentComplete()` default the select away from `Gatesale` in that case, with `submitGatesaleRegistration()` additionally short-circuiting client-side (`alert(...)`, no fetch sent) as a defensive second layer. This is a UX improvement only — it stops a visitor from filling out the whole registration form only to be rejected at Save, but the **backend 403 in `gatesaleRegisterIdentity()`/`createGatesaleVisit()`/`handleSelfServiceRecognition()` remains the actual, load-bearing enforcement** and must never be removed on the assumption the disabled `<option>` is sufficient by itself (a disabled `<option>`'s value can still be set programmatically/via devtools).
- **Truck facility eligibility (`facility_list.is_truck`, added 2026-09-03, later the same day as `is_gs`) — deliberately mirrors the `is_gs` design point-for-point, as an independent flag rather than a reuse of `is_gs`.** A facility must have `is_truck = 1` before its kiosk will allow the Truck self-service process; `is_gs` and `is_truck` are fully independent (a facility can allow Gatesale only, Truck only, both, or neither). Enforced via the shared `KioskController::isTruckEligibleFacility(KioskDevice $kiosk): bool` helper (`(bool) $kiosk->facility?->is_truck` — same null-safe fail-closed behavior as `isGatesaleEligibleFacility()`), at the identical three direct entry points:
  - `handleSelfServiceRecognition()` — checked only in the "no ACTIVE request anywhere" branch, for a directory whose own `visitor_type` is Truck. An already-ACTIVE request is resumed unconditionally, never retroactively blocked by a later flag flip — identical to the Gatesale case.
  - `createGatesaleVisit()` (the shared visit-creation method for both self-service types) — checked only in the "no existing active request, about to create a brand-new one" branch, for a Truck directory.
  - `gatesaleRegisterIdentity()` — checked only in the "no face match, registering a brand-new identity" branch, when the **requested** `visitor_type` is Truck, before the transaction opens. The "matched an existing directory" branch re-routes through `handleSelfServiceRecognition()`, which checks eligibility against that directory's *actual* type — identical to Gatesale.
  - `gatesaleUpdateDetails()` is, for the same reasoning as Gatesale, deliberately **not** gated for Truck either.
  - Failure shape: `403 {success: false, type: 'truck_not_available', message: 'Truck self-service is not available at this facility.'}` from `handleSelfServiceRecognition()`/`gatesaleRegisterIdentity()`; a plain `abort(403, '...')` from inside `createGatesaleVisit()`'s transaction closure — same pattern as Gatesale.
  - Every existing facility defaults to `is_truck = false` (the migration's column default, same as `is_gs`) — Truck self-service is disabled everywhere until an admin explicitly re-enables it per facility.
  - **Frontend enforcement:** the Truck `<option>` in `#gatesale-reg-type` gets the same `disabled` treatment (driven by `$kiosk->facility?->is_truck`), a matching JS `const truckEnabled` flag, and `submitGatesaleRegistration()`'s client-side short-circuit extended to Truck — same UX-convenience-only caveat as Gatesale's: the backend 403 is the real enforcement.
  - **Do not extend `is_truck`'s reach beyond these three entry points, and do not merge it back into `is_gs`** — they are deliberately independent flags/helpers/response types (`truck_not_available` vs `gatesale_not_available`) even though the logic is structurally identical, since a facility's Gatesale and Truck eligibility are genuinely separate admin decisions.
...
**Error Handling:** `resolveSelfServiceDirectoryOrFail` aborts 422 on any identity mismatch. `createGatesaleVisit` aborts 403 if an active request exists at a different farm (checked again inside the locked transaction, not just at recognize time), 403 if a genuinely new Gatesale visit is attempted at a facility with `is_gs = false`, and (added 2026-09-03) 403 if a genuinely new Truck visit is attempted at a facility with `is_truck = false`. Missing Host/Origin/Purpose for a genuinely new visit → 422. `facility_list.is_break_enabled = false` additionally rejects a `temporary_exit` on any self-service visit the same way it does for Visitor-with-Approval — see "Kiosk Entry"'s Business Rules for the full break-restriction design (it lives in the shared `processEntry()`, not in this module's own code).

**Important Files:**

| Component | File | Responsibility |
|---|---|---|
| Controller | `app/Http/Controllers/Kiosk/KioskController.php` | `gatesaleUpdateDetails`, `gatesaleCreateVisit`, `gatesaleRegisterIdentity` (takes `poses` as of 2026-09-03), `routeByIdentity`, `handleSelfServiceRecognition`, `createGatesaleVisit`, `recognize` (uses `match()` as of 2026-09-03), `isGatesaleEligibleFacility`/`gatesaleNotEligibleResponse` (the `is_gs` gate, added 2026-09-03), `isTruckEligibleFacility`/`truckNotEligibleResponse` (the `is_truck` gate, added 2026-09-03, mirrors the `is_gs` gate exactly) (same file as Kiosk Entry); `show()` eager-loads `$kiosk->load('facility')` (added 2026-09-03) so the view can read `is_gs`/`is_truck`/`is_break_enabled` without a lazy-load |
| Model | `app/Models/FacilityList.php` | `is_gs` (boolean, default `false`, added 2026-09-03) — Gatesale eligibility; `is_truck` (boolean, default `false`, added 2026-09-03) — Truck eligibility, independent of `is_gs`; `is_break_enabled` (boolean, default `true`, added 2026-09-03) — see "Facility Master Data" for all three |
...
| View | `resources/views/kiosk/show.blade.php` | The `#gatesale-reg-type` select's Gatesale `<option disabled>` (server-side, from `$kiosk->facility?->is_gs`) and the `gatesaleEnabled` JS flag (added 2026-09-03) — see the `is_gs` Business Rules bullet above for the full mechanism. Also: `.video-section` max-width `700px` (main recognition) and `#enrollment-webcam`'s wrapper max-width `700px` (Face Enrollment/registration) are now deliberately equal — a first pass (2026-09-03) bumped these up unevenly (`700px`/`420px`), which real usage then showed made registration's camera preview visibly smaller than recognition's; a same-day follow-up set both to `700px` and widened the enclosing gatesale-view `.setup-prompt` panel to `760px` (30px padding either side of the 700px video) so both cameras render at the same physical size. Plain CSS sizing only, no behavior change |
...
| JS/CSS | `public/js/face-guide-ui.js`, `public/css/face-guide.css` (new 2026-09-03) | Purely visual ring/scan-line/hint/badge overlay for the same `#enrollment-webcam` screen — created lazily on first "Registration" tap in `startGatesaleRegistration()`, reset in `gatesaleCancelRegistration()`/the idle-timeout callback, updated inside `onGatesaleEnrollmentStateChange()`; no detection/capture logic of its own |

**Configuration:** None specific; relies on the same `CACHE_STORE=database` used app-wide. `facility_list.is_gs`/`is_truck`/`is_break_enabled` are per-facility data (admin-managed via `admin/facility-configuration`, and `is_gs`/`is_break_enabled`/`is_truck` are also all settable individually via the plain `admin/facilities` edit form's own checkboxes for `is_gs` — `is_break_enabled`/`is_truck` are, by contrast, editable **only** through `admin/facility-configuration`, not the Facility edit form; see "Admin Management"'s Facility Configuration subsection), not an env var/config key.

**Known Constraints:** **`VisitorType` rows for "Gatesale" and "Truck" (and "Visitor") are not created by any seeder** (`database/seeders/` has no `VisitorTypeSeeder`) and there is **no admin CRUD controller for `VisitorType`** either (unlike `IdentityType`/`EmployeeType`, which both have full admin UIs). `NEEDS VERIFICATION`: how these rows are provisioned in a real deployment — likely manual DB insert or `tinker`, since tests create them ad hoc (`VisitorType::create(['visitor_type_name' => 'Gatesale'])`). This is a real operational gap worth flagging if asked to touch onboarding/setup. Same guided-capture tuning/no-JS-test-runner caveats noted under "Visitor Registration" apply here too, since both pages share the identical `face-enrollment.js` module. **The kiosk frontend disables both the Gatesale and Truck options server-side** (Gatesale added 2026-09-03 same day as `is_gs`; Truck added 2026-09-03 later the same day as `is_truck`, mirroring it exactly) — see Important Files/UI enforcement above; backend rejection (`gatesale_not_available`/`truck_not_available`) remains the actual, load-bearing enforcement regardless of what the disabled `<option>`s suggest.

**Important Notes for Future Changes:** Do not scope the "one active request" check by farm or date — it is deliberately global-and-status-only. Do not change the `Cache::lock()` key to include farm — that would reopen the cross-farm race it was added to close. If you add a new self-service visitor type, it must flow through `handleSelfServiceRecognition`/`resolveSelfServiceDirectoryOrFail`, not a parallel implementation. Do not duplicate `face-enrollment.js`'s guided-capture logic inline in this file — if the kiosk registration UX ever needs to diverge from the visitor-registration one, extend the shared module's options/callbacks rather than forking it. The same applies to the visual scan overlay — change `public/js/face-guide-ui.js`/`public/css/face-guide.css` (added 2026-09-03), not a page-local fork; it stays presentation-only, driven by `face-enrollment.js`'s existing `onStateChange`. **`is_truck` (added 2026-09-03) now gates Truck the same way `is_gs` gates Gatesale** — this was a deliberate follow-up decision, not a default; do not merge the two flags/helpers/response-type strings back into one, since a facility's Gatesale and Truck eligibility remain genuinely independent admin choices (see the `is_truck` bullet above for the full mirrored design). If a third self-service visitor type is ever added, treat its eligibility gate the same deliberate way — a new flag/helper pair, not a reuse of `is_gs` or `is_truck`.

---

### Module: `Face Matching` (shared service) — Multi-Pose Upgrade (Phases A–D added 2026-09-03; Phase E not done)
...
**Current status:** Phase A (child table + scoring algorithm), Phase B (all 4 call sites switched over), Phase C (guided multi-pose capture frontend, see Visitor Registration/Kiosk Self-Service — including a same-day fix for an inverted LEFT/RIGHT yaw mapping found during Phase C's own manual testing), and Phase D (same-day audit/cleanup/verification pass — confirmed the architecture and backward compatibility hold end-to-end, removed one unused export and two stale comments, full test suite re-run green) have all shipped. **Phase E has not shipped**: there is no backfill command yet to give existing legacy `face_profile` rows an explicit `face_profile_embedding` child row (they still match correctly without one — see Business Rules — this is a data-tidiness/future-reporting gap, not a correctness gap), no shared `FaceProfileFactory`/`CreatesFaceProfiles` test trait (each test file still hand-rolls its own `descriptor()` helper, unchanged from before this upgrade), and the ambiguity margin (`0.08`) has not been recalibrated against any real distance-distribution data. Phase D's audit also surfaced one minor, deliberately-untouched finding: `capture.blade.php` and `kiosk/show.blade.php` each independently define their own pose-instruction display-text mapping rather than sharing one source of truth — flagged for a future decision, not fixed automatically, since consolidating them would mean changing which file owns that text (an architectural call, not pure cleanup).
...
**Main Flow:**
```text
R~~esolveExpiredVisitorSessions::handle
   ↓
...
### Module: `Facility Master Data` (Phase 1: additive tables — Phase 2: core cutover — Phase 3: admin CRUD — Phase 4: Downtime Matrix/Stationary cutover — Phase 5: legacy Farm admin decommissioned — all 2026-08-27; Phase 6: `is_gs` Gatesale-eligibility flag added 2026-09-03; Phase 7: `is_break_enabled`/`is_truck` columns + centralized Facility Configuration admin module added 2026-09-03; Phase 8: `is_truck` Truck-eligibility enforcement, mirroring `is_gs`, added 2026-09-03)

**Purpose:** Normalized, multi-brand replacement for the farm-only `farm_list`/`farm_aliases` model — introduced so that non-farm sites (plants, DC warehouses) can eventually be represented alongside farms, classified by business group (`facility_type`: BVA/GEFI/GEFI-LIVE/PS/FEEDMILL/GP-HY/PS-HY/IBG) and physical kind (`facility_category`: FARM/PLANT/DC_WAREHOUSE/OTHER), with an `is_rtl` flag and (as of Phase 6) an `is_gs` flag per facility.
...
- **Phase 6** (2026-09-03) added a single additive column, `facility_list.is_gs` (boolean, non-nullable, default `false`) — a plain per-facility attribute, **not** a new table/relationship and **not** a change to `facility_type`/`facility_category`. It controls eligibility for the Kiosk Self-Service **Gatesale** process only (Truck was, at this point, still unaffected — see Phase 8 below for when that changed). Every existing facility defaulted to `false` on migration — there was no existing reliable signal (no prior "is this a Gatesale farm" flag anywhere in the schema) to seed `true` from, so this is a genuine "Gatesale starts disabled everywhere, an admin re-enables per facility" cutover. The admin `Facility` create/edit forms (Phase 3's CRUD) got a matching "Gatesale Enabled" checkbox and Data Table column in the same change, mirroring the existing `is_rtl` pattern exactly — this is one of two ways to set `is_gs` today (see Phase 7 for the other).
- **Phase 7** (2026-09-03, same day, later) added two more additive columns — `facility_list.is_break_enabled` (boolean, non-nullable, **default `true`** — the one exception to "everything starts disabled": since unlimited intermediate breaks during one visitation was already the universal existing behavior, defaulting this `false` would have been a silent breaking change, not a safe opt-in) and `facility_list.is_truck` (boolean, non-nullable, default `false`, mirroring `is_gs`'s polarity) — and a new centralized **Facility Configuration** admin module (`admin/facility-configuration`, reusing the `facilities.manage` permission rather than a new key) so an admin can toggle Gatesale/Break Enabled/Truck for every facility from one checkbox-grid screen instead of opening each facility's own edit form. Unlike `is_gs` (settable both from the plain Facility edit form's checkbox *and* from this new module), `is_break_enabled`/`is_truck` are editable **only** through `admin/facility-configuration` — the Facility create/edit forms were deliberately left untouched for these two, to avoid two divergent edit surfaces for genuinely new fields. `is_break_enabled` is consumed by `VisitorKioskService::processEntry()` (see "Kiosk Entry"'s Business Rules) — at this Phase 7 point `is_truck` was still purely structural/unenforced.
- **Phase 8** (2026-09-03, same day, later still) wired up **`is_truck` enforcement**, deliberately mirroring the `is_gs`/Gatesale gate point-for-point rather than reusing it — `KioskController::isTruckEligibleFacility()`/`truckNotEligibleResponse()`, checked at the same three direct entry points (`handleSelfServiceRecognition()`, `createGatesaleVisit()`, `gatesaleRegisterIdentity()`) that `is_gs` already used, plus the matching kiosk frontend `<option disabled>`/`truckEnabled` JS treatment. See "Kiosk Self-Service"'s Business Rules for the full mirrored design and exactly why it's a separate flag/helper pair rather than a generalization of `is_gs`.

**Responsibilities (today):** Canonical facility-name resolution for Visitor Sync (via `FacilityResolver`); the FK target for `visitor_request.facility_id`, `kiosk_device.facility_id`, `downtime_matrix.{origin,destination}_facility_id`, and `downtime_stationary.assigned_facility_id`; full admin CRUD for facilities and facility aliases; the source of truth (`is_gs`/`is_truck`) for Kiosk Self-Service's Gatesale/Truck eligibility gates, and (`is_break_enabled`) for Kiosk Entry's break restriction.
...
- **`is_gs`/`is_break_enabled`/`is_truck` (Phases 6–7) are plain, independent boolean attributes of `facility_list` — none of them interact with `facility_type`/`facility_category`/`is_rtl`/each other in any way and none must be inferred from another.** A facility's Gatesale eligibility, Truck eligibility, and break-restriction setting say nothing about what kind of facility it is, nor about each other — do not assume, for example, that every `FARM` category facility should default to `is_gs`/`is_truck = true`, or that enabling Gatesale implies anything about Truck or breaks — each is purely an explicit, independent admin choice per facility, same as `is_rtl`.

**Models:** `App\Models\FacilityType`, `App\Models\FacilityCategory`, `App\Models\FacilityList` (relations: `facilityType()`, `facilityCategory()`, `aliases()`, `kioskDevices()`, `originDowntimeMatrixRules()`, `destinationDowntimeMatrixRules()`, `downtimeStationaryRules()`; `is_gs` boolean attribute (Phase 6), `is_break_enabled`/`is_truck` boolean attributes (Phase 7)), `App\Models\FacilityAlias` (relation: `facility()`) — all four use the `Auditable` trait (matching `FarmList`/`FarmAlias`/`IdentityType`/`EmployeeType`'s pattern). `App\Models\VisitorRequest::facility()`, `App\Models\KioskDevice::facility()`, `App\Models\DowntimeMatrix::originFacility()`/`destinationFacility()`, and `App\Models\DowntimeStationary::assignedFacility()` (all renamed from their `*Farm()` equivalents) now belong to `FacilityList`, not `FarmList`. `FarmList`'s reciprocal `kioskDevices()`/`originDowntimeMatrixRules()`/`destinationDowntimeMatrixRules()`/`downtimeStationaryRules()` methods were removed (Phase 4) since the columns they pointed at no longer exist on those tables — moved to `FacilityList` instead.
...
| Migration (Phase 6) | `database/migrations/2026_09_03_110000_add_is_gs_to_facility_list_table.php` | Additive `facility_list.is_gs` boolean, default `false`, fully reversible |
| Migration (Phase 7) | `database/migrations/2026_09_03_120000_add_is_break_enabled_and_is_truck_to_facility_list_table.php` | Additive `facility_list.is_break_enabled` (default `true`) + `is_truck` (default `false`) booleans, fully reversible |
| Controller (Phase 7) | `app/Http/Controllers/Admin/FacilityConfigurationController.php` | `index`/`data`/`update` only (no create/store/destroy) — the centralized checkbox-grid editor for `is_gs`/`is_break_enabled`/`is_truck`, reuses `facilities.manage` |
| Request (Phase 7) | `app/Http/Requests/Admin/UpdateFacilityConfigurationRequest.php` | Validates `field` against an `in:is_gs,is_break_enabled,is_truck` allow-list + `value` as boolean |
| Views (Phase 7) | `resources/views/admin/facility-configuration/{index,_index}.blade.php` | Admin UI — one Data Table, one row per facility, three toggle columns, each checkbox PATCHes immediately on change |
...
| Tests (Phase 6) | `tests/Feature/Admin/FacilityAdminTest.php` (`is_gs` create/update coverage), `tests/Feature/Kiosk/KioskGatesaleFlowTest.php` and `KioskTruckFlowTest.php` (eligibility gate coverage — see "Kiosk Self-Service") | `is_gs` field validation/persistence, and the actual Gatesale-process enforcement |
| Tests (Phase 7) | `tests/Feature/Admin/FacilityConfigurationAdminTest.php` (index shell, `/data` envelope + search filter, per-field `update` toggling for all three fields, allow-list rejection, permission gating); `tests/Feature/Kiosk/KioskEntryTest.php`/`KioskGatesaleFlowTest.php`/`KioskRecognizeTest.php` (`is_break_enabled` coverage — `temporary_exit` rejection, `break_enabled` response field, already-started-break-survives-a-later-flag-flip) | Facility Configuration module + break restriction |
| Tests (Phase 8) | `tests/Feature/Kiosk/KioskTruckFlowTest.php` (`is_truck` eligibility gate — recognition/create-visit/register-identity rejection, already-active-visit-unaffected-by-later-flip, Gatesale-unaffected-by-`is_truck`, no-resolvable-facility-fails-closed) | Truck-process enforcement, mirroring Phase 6's Gatesale coverage exactly |

**Known Constraints:** `facility_code` is real for the 8 farm-matched facilities but still placeholder (`TYPE-SLUG`) for the other 8 (no requirement to change these — the admin screen lets anyone rename them now that a UI exists). `facility_aliases` has zero rows, so any AppSheet facility-name spelling other than the exact/normalized match will fail sync with "Facility not found" until an admin adds an alias via `admin/facility-aliases`. `farm_list`/`farm_aliases` are now pure legacy data with **no admin UI at all** (Phase 5 removed it) — they exist only as a rollback/legacy safety layer per the source instructions; nothing in the running application reads or writes them. `resources/views/admin/dashboard{,-content}.blade.php` still show a "Farms" stat tile (`FarmList::count()`) — a known, deliberately-left residual (see Phase 5 note above), harmless but pointing at a now admin-less concept. **Every facility defaults to `is_gs = false` and `is_truck = false` (Phases 6/8)** — until an admin explicitly re-enables specific facilities via `admin/facility-configuration` (or, for `is_gs` only, the plain Facility edit form), the Kiosk Self-Service Gatesale/Truck processes are disabled everywhere; this is an intended, not accidental, consequence of the safe-default requirement (see "Kiosk Self-Service"'s Business Rules), but is worth flagging proactively after either migration runs against a real environment, since it is an immediate behavior change for any facility that was actually using Gatesale or Truck before. **`is_break_enabled` defaults to `true` (Phase 7)** — the opposite polarity, deliberately, since it preserves rather than restricts existing behavior; see "Kiosk Entry"'s Business Rules.

**Important Notes for Future Changes:** The only remaining farm→facility decision is whether/when to drop the now fully-legacy `farm_list`/`farm_aliases` tables themselves (kept intentionally as of Phase 5, per the source instructions' rollback guidance — nothing reads or writes them, admin UI included). Do not re-derive the farm_id→facility_id mapping from name matching in new code — treat the table above as the historical record of a fact, not a formula. When adding fields to the Facility form, follow `FacilityController`'s exact pattern (Store/Update Request + view three-way split) — `is_gs` (Phase 6) is the most recent example of this pattern; `is_break_enabled`/`is_truck` (Phase 7) deliberately did **not** follow it, living only in the new Facility Configuration module instead — if you add a fourth per-facility runtime toggle, default it to the Facility Configuration screen unless there's a specific reason it also needs to live on the plain Facility form. **If you ever write another migration that renames a FK column backed by a unique/composite-unique index on MySQL**, drop the FK in its own prior `Schema::table()` statement before touching the index or column (InnoDB blocks dropping an index a live FK still needs), and check any auto-generated constraint name against MySQL's 64-character identifier limit before relying on it — both bit the Phase 4 migration and neither was caught by the sqlite-backed test suite. `is_gs` and `is_truck` (Phase 8) are now both real, independent eligibility gates — do not merge them into one flag, and if a third self-service visitor type is ever added, give it its own new flag/helper pair the same deliberate way rather than reusing either.
...
| Facility Configuration (added 2026-09-03, Facility Master Data Phase 7) | `FacilityList` (same model as Facilities — a different UI over the same table's `is_gs`/`is_break_enabled`/`is_truck` columns) | `facilities.manage` (reused, not a new key — this is a UI convenience over the same resource, not a new authorization boundary) |
...
**Admin Data Table Architecture — jQuery DataTables.js, filter-button-gated, server-side processing (added 2026-08-28; this is now the ONE standard, uniformly applied to all 11 original admin listings, plus Facility Configuration added 2026-09-03 as a 12th, update-only variant):**

Every admin resource listing — Facilities, Facility Aliases, Kiosk Devices, Identity Types, Employee Types, Downtime Matrix, Downtime Stationary, the Downtime Matrix Import *list*, **and (added later the same day) the Downtime Matrix Import *Preview/show* page's three category tabs**, Roles, Users, and Audit Logs — runs this exact pattern. **Facility Configuration (added 2026-09-03) reuses the identical shell/filter/`/data`-endpoint pattern, with one deliberate difference: it has no create/edit/destroy at all** — each row's three checkbox cells PATCH `admin/facility-configuration/{facility}` directly on `change`, with no separate save action and no `renderActions()` column; it is the first admin Data Table in this codebase built purely as an in-place bulk toggle editor rather than a CRUD listing. (Two earlier same-day iterations existed during development — a fully server-rendered `<table>`, then a custom hand-rolled Data Table that auto-loaded on page open — both fully replaced; nothing in the running app uses either anymore, and `public/js/admin-datatable.js` plus `RendersDataTableJson.php` were deleted, not just deprecated.) The Preview page is the one case where a single page hosts **three independent Data Tables** (one per tab, all backed by one `rows-data` endpoint filtered by `rule_type`) rather than one — see the "Downtime Matrix PDF Import" module's own section for that page's specifics; everything below still applies per-tab.
...
- **`facility_type`** / **`facility_category`** / **`facility_list`** / **`facility_aliases`** (new tables, 2026-08-27 — see the "Facility Master Data" module above) — Purpose: normalized, multi-brand replacement structure for `farm_list`/`farm_aliases`, seeded with real reference data (8 types, 4 categories, 16 facilities). **As of the same-day Phase 2 cutover, this is now the live, authoritative site-binding target** for `visitor_request.facility_id` and `kiosk_device.facility_id`, and the resolution target for Visitor Sync's `FacilityResolver`. `facility_list.facility_code` is real for the 8 facilities that correspond to a `farm_list` row, still placeholder (`TYPE-SLUG`) for the other 8. `facility_list`/`facility_aliases` got a full admin CRUD in Phase 3 (2026-08-27) — see the module section (this note predates that phase and is corrected here). `facility_list.is_gs` (new column, added 2026-09-03) is a plain non-nullable boolean, default `false`, gating whether the **Gatesale** self-service visitor process (Kiosk Self-Service) is allowed at that facility; `facility_list.is_truck` (added 2026-09-03, later the same day) is the independent, analogous flag for the **Truck** process, mirroring `is_gs` exactly rather than being inferred from it; `facility_list.is_break_enabled` (added 2026-09-03, same day) is a third, independent boolean, default `true`, gating whether Kiosk Entry's intermediate "Go Outside"/`temporary_exit` option is available for a visitation at that facility — see the "Kiosk Self-Service"/"Kiosk Entry" modules' Business Rules for the enforcement points and "Facility Master Data"'s Known Constraints for the default-value reasoning behind each.
...
27. **Gatesale self-service requires `facility_list.is_gs = true` on the kiosk's own facility** (Kiosk Self-Service, added 2026-09-03): enforced server-side at every direct entry point (`handleSelfServiceRecognition()` for recognition-time routing, `createGatesaleVisit()` for visit creation, `gatesaleRegisterIdentity()` for new-identity registration — never only via UI hiding), and only gates *starting* a new visit/identity, never disrupts a visit already in progress if the flag is later turned off. Every facility defaults to `is_gs = false`. `is_gs` and `is_truck` (rule 29 below) are fully independent flags — do not merge them.
28. **A break-disabled facility (`facility_list.is_break_enabled = false`) permits exactly one IN → one OUT per visitation, evaluated per visitation+facility, never globally per person** (Kiosk Entry, added 2026-09-03): `VisitorKioskService::processEntry()` rejects `temporary_exit` server-side whenever the visit's own facility has this flag off — this is the real enforcement, independent of what the kiosk frontend renders (it also hides the "Go Outside" button via a `break_enabled` field on the `/recognize` response, but that's UX only). Only *starting* a new break is gated — `return`/`final_exit` are never blocked by this flag, and a break already in progress when the facility was break-enabled is never retroactively invalidated by a later flag flip. Every facility defaults to `is_break_enabled = true` (the opposite polarity from `is_gs`/`is_truck`), since unlimited breaks was already the universal pre-existing behavior and this rule is an opt-out restriction, not an opt-in feature.
29. **Truck self-service requires `facility_list.is_truck = true` on the kiosk's own facility, enforced exactly like rule 27's Gatesale gate** (Kiosk Self-Service, added 2026-09-03): same three entry points (`handleSelfServiceRecognition()`, `createGatesaleVisit()`, `gatesaleRegisterIdentity()`), same "only gates starting a new visit/identity" scope, same `false` default. `is_gs` and `is_truck` are independent — a facility can allow Gatesale only, Truck only, both, or neither. Do not merge them into one flag, and give any future third self-service visitor type its own new flag/helper pair the same deliberate way rather than reusing either.
...
| Visitor Registration | `app/Http/Controllers/Visitor/RegistrationController.php`, `app/Services/VisitorRegistrationService.php`, `app/Services/Qr/VisitorQrCodeService.php`, `public/js/face-enrollment.js` (shared with Kiosk Self-Service), `public/js/face-guide-ui.js`/`public/css/face-guide.css` (shared visual overlay, added 2026-09-03), `resources/views/visitor/capture.blade.php` |
...
| Kiosk Self-Service | `app/Http/Controllers/Kiosk/KioskController.php` (same file as Kiosk Entry), `app/Models/VisitorProfile.php`, `public/js/face-enrollment.js` (shared with Visitor Registration), `public/js/face-guide-ui.js`/`public/css/face-guide.css` (shared visual overlay, added 2026-09-03), `resources/views/kiosk/show.blade.php` |
...
| Facility Master Data | `app/Models/{FacilityType,FacilityCategory,FacilityList,FacilityAlias}.php`, `app/Services/FacilityResolver.php`, `app/Http/Controllers/Admin/{Facility,FacilityAlias,FacilityConfiguration}Controller.php`, `database/seeders/Facility{Type,Category,List}Seeder.php`, `database/migrations/2026_08_27_110000_*` through `..._120002_*`, `..._2026_09_03_110000_add_is_gs_*`, `..._2026_09_03_120000_add_is_break_enabled_and_is_truck_*` |
...
8. Do not change any of the 27 business rules in §9 unless explicitly asked to.
...
| 13 | Face Matching / Visitor Registration / Kiosk Self-Service | Multi-Pose Face Enrollment upgrade (2026-09-03) shipped Phases A–D only — Phase E (legacy-profile backfill Artisan command, shared `FaceProfileFactory`/`CreatesFaceProfiles` test trait, ambiguity-margin recalibration against real distance data) has not been done | Legacy `face_profile` rows still match correctly (fallback to their own `embedding` column — this is not a correctness bug), but have no explicit `face_profile_embedding` row for future admin/reporting tooling to query against; the `0.08` ambiguity margin is an unverified starting guess; each face-matching test file still hand-rolls its own local `descriptor()` helper (unchanged, pre-existing pattern) | None currently — purely a data-tidiness/tooling gap and a calibration task, not a functional defect | `app/Services/Face/FaceMatchingService.php`, `app/Models/FaceProfileEmbedding.php` | CONFIRMED (deliberately deferred, per the approved phased implementation plan; re-confirmed still deferred by the Phase D 2026-09-03 audit) |
| 15 | Visitor Registration / Kiosk Self-Service (guided capture) | `capture.blade.php` and `kiosk/show.blade.php` each independently define their own pose-instruction display-text mapping (`STATE_MESSAGES` / `GATESALE_CAPTURE_MESSAGES`) instead of sharing one source of truth — found during the Phase D 2026-09-03 audit | Editing one page's wording (e.g. "Slowly turn your head left.") does not update the other page's copy of the same text; both currently say the same thing by coincidence of having been written together, not by a shared reference | Low — cosmetic/maintainability only; no functional or data-integrity impact | `resources/views/visitor/capture.blade.php`, `resources/views/kiosk/show.blade.php` | CONFIRMED (deliberately left alone — consolidating would mean changing which file owns the display text, an architectural call the audit was not authorized to make automatically) |
| 14 | Visitor Registration / Kiosk Self-Service (guided capture) | The guided FRONT/LEFT/RIGHT capture's pose/position **threshold magnitudes** (`public/js/face-enrollment.js`'s `FaceEnrollment.config`: yaw proxy, centering/size ratios, stability window) are reasoned starting defaults, not calibrated against real enrollment footage; there is also no JS test runner in this repo (`package.json` only lists `jquery`) so the guided-capture state machine has no automated test coverage of its own | Pose detection may feel too strict/loose in real use until tuned; a regression in the module's detection logic would only surface via manual testing, not a test suite | Enrollment UX quality risk until a tuning pass happens against real usage; not a data-integrity risk, since the *backend* `poses` contract the module feeds into is fully tested | `public/js/face-enrollment.js`, `resources/views/visitor/capture.blade.php`, `resources/views/kiosk/show.blade.php` | CONFIRMED (explicitly flagged as a tuning item when the feature shipped); **the LEFT/RIGHT sign mapping itself (a distinct issue from magnitude tuning) was found inverted during manual Phase C testing and fixed same-day — see `yawMatchesTarget()`'s docblock for the corrected raw-frame-vs-mirrored-preview geometry** |
...
If the request is about the Kiosk (recognition, entry/exit, Gatesale/Truck):
    ...
    ↓
    If the change touches Gatesale specifically, check isGatesaleEligibleFacility()/
    facility_list.is_gs (added 2026-09-03). If it touches Truck specifically, check
    isTruckEligibleFacility()/facility_list.is_truck (added 2026-09-03, mirrors is_gs
    exactly at the same three entry points) - the two flags are independent, do not
    merge them or infer one from the other
    ↓
    If the change touches the intermediate-break behavior (Go Outside/temporary_exit),
    check facility_list.is_break_enabled and the guard in
    VisitorKioskService::processEntry() (added 2026-09-03) - it applies to every
    visitor type sharing processEntry(), is evaluated per visitation+facility (never
    globally per person), and must never retroactively invalidate a break already
    started while the facility was break-enabled
    ↓
...
### Modules
Visitor Sync · Visitor Registration (guided multi-pose capture added 2026-09-03; `FaceGuideUI` visual scan overlay added 2026-09-03) · Kiosk Entry (break restriction via `facility_list.is_break_enabled` added 2026-09-03) · Kiosk Self-Service (Gatesale/Truck, guided multi-pose capture added 2026-09-03; Gatesale facility-eligibility flag added 2026-09-03; Truck facility-eligibility flag, mirroring Gatesale's, added 2026-09-03; `FaceGuideUI` visual scan overlay added 2026-09-03) · Face Matching (multi-pose scoring/ambiguity upgrade, Phases A–D added 2026-09-03, Phase E not done) · Google Sheets Integration · Session Auto-Resolution · Facility Master Data (live site-binding target since the 2026-08-27 cutover; `is_gs`/`is_break_enabled`/`is_truck` flags added 2026-09-03) · Admin Management (Facility Configuration checkbox-grid module added 2026-09-03) · Downtime Matrix PDF Import (Phase 1 — parse/validate/preview, added 2026-08-27; Phase 2 — Save to Production mapping into `downtime_matrix`/`downtime_stationary`, added 2026-08-28) · Authentication & Authorization · Audit Logging (cross-cutting)

### Main APIs
`POST /api/v1/visitor/sync` · `POST /kiosk/{kiosk}/recognize` (returns a `break_enabled` field as of 2026-09-03) · `POST /kiosk/{kiosk}/entry` (`temporary_exit` rejected when the facility disables breaks, as of 2026-09-03) · `POST /kiosk/{kiosk}/gatesale/{update-details,create-visit,register-identity}` (accepts `poses` as of 2026-09-03; `create-visit`/`register-identity` reject Truck too when `is_truck` is off, as of 2026-09-03) · `/register/visitor/*` (public; `capture` accepts `poses` as of 2026-09-03) · `/admin/*` (authenticated resource routes, including `admin/facility-configuration` added 2026-09-03) · `/admin/biosecurity-rules/downtime-matrix-import/*` (upload/preview/verify/cancel) · `/login`, `/logout`

### Main Data Entities
`user_directory` · `visitor_profile` · `face_profile` (identity-level; `embedding`/`face_image` are the FRONT-pose fallback as of 2026-09-03) · `face_profile_embedding` (new 2026-09-03 — one row per FRONT/LEFT/RIGHT pose, child of `face_profile`) · `visitor_request` (site-bound via `facility_id`) · `visitor_session` · `visitor_entry_logs` · `kiosk_device` (site-bound via `facility_id`) · `facility_type` / `facility_category` / `facility_list` (`is_gs` boolean, added 2026-09-03, gates Kiosk Self-Service's Gatesale process; `is_truck` boolean, added 2026-09-03, gates Truck the same way, fully independent of `is_gs`; `is_break_enabled` boolean, added 2026-09-03, defaults `true`, gates Kiosk Entry's intermediate-break option) / `facility_aliases` (live site-binding target for `visitor_request`, `kiosk_device`, `downtime_matrix`, and `downtime_stationary` since 2026-08-27, now with a full admin CRUD) · `farm_list` / `farm_aliases` (pure legacy data as of 2026-08-27 Phase 5 — no admin UI, no remaining FKs, `FarmList`/`FarmAlias` models kept but unused) · `identity_type` / `employee_type` / `visitor_type` · `downtime_matrix` / `downtime_stationary` (Biosecurity Rules submodules, formerly `biosecurity_rules`, facility-based since the 2026-08-27 Phase 4 cutover) · `downtime_matrix_imports` / `downtime_matrix_import_rows` (Downtime Matrix PDF Import staging tables, added 2026-08-27 — never connected to `downtime_matrix`/`downtime_stationary`) · `role` / `permission` / `users` · `audit_logs` / `api_logs`

### External Systems
AppSheet (inbound webhook, one-way) · Google Sheets API (outbound write, one-way) · `face-api.js` + `jsQR` (client-side, CDN-loaded)

### Critical Business Rules
Directory merge requires full_name+email match · No fuzzy farm matching · Terminal request states are permanent · Farm binding double-enforced · Gatesale/Truck: one active visit globally, guarded by a directory-keyed lock · Google Sheets writes excluded for Gatesale/Truck and always best-effort/non-blocking · Session Auto-Resolution never fabricates recovered times · Biometric conflict never blocks QR entry · RBAC checked at both route and FormRequest layers · Downtime Matrix/Stationary uniqueness enforced at both DB and FormRequest layers · Downtime Matrix PDF Import only writes to `downtime_matrix`/`downtime_stationary` through a confirmed Save to Production step (never through Verify alone), maps only VALID/WARNING rows, and never assigns a facility-group match a single `facility_id` — a group is expanded into its current active member facilities only at production-mapping time · Face matching scores by minimum distance across a profile's available poses, never an average · a poses-based enrollment write path never fabricates a pose it wasn't given · face-match ambiguity handling is decided deliberately per call site (fail-safe for kiosk recognition, route-to-human-confirmation for dedup checks) · legacy single-embedding face profiles remain matchable forever, with no forced migration · Gatesale self-service requires `facility_list.is_gs = true` on the kiosk's facility, enforced server-side at every direct entry point · Truck self-service requires `facility_list.is_truck = true`, enforced identically and independently of `is_gs` · a break-disabled facility (`facility_list.is_break_enabled = false`) permits exactly one IN/one OUT per visitation, enforced server-side in `processEntry()` regardless of what the kiosk UI shows, evaluated per visitation+facility never globally, and never retroactive on an already-started break

### Critical Constraints
Face matching is an unindexed linear PHP scan (now up to 3x pose-comparison volume for fully multi-pose-enrolled profiles) · Kiosk recognition depends on external CDNs with no offline fallback · Google Sheets has no batching, only a 3x retry · `audit_logs`/`api_logs` have no pruning · `VisitorType` has no seeder/admin UI (operational gap, unlike `facility_list`, which got one 2026-08-27) · every Admin controller's `view()` fallback for `create()`/`edit()` on a non-AJAX request throws (never triggered in real use, see Known Issue #11) · Downtime Matrix PDF Import's grid-reconstruction tolerances are tuned against one real PDF layout and its parsing is synchronous/single-page only · the guided multi-pose enrollment capture's pose/position threshold *magnitudes* are reasoned starting defaults, not calibrated against real footage, and this repo has no JS test runner to cover that logic (see Known Issue #14; the LEFT/RIGHT *sign* mapping itself was fixed same-day) · no Phase E backfill command exists yet for legacy `face_profile` rows (see Known Issue #13) · `capture.blade.php`/`kiosk/show.blade.php` duplicate their pose-instruction display text rather than sharing one source (see Known Issue #15) · every facility defaults to `facility_list.is_gs = false` and `is_truck = false` (added 2026-09-03), so Gatesale/Truck self-service are both disabled everywhere until an admin explicitly re-enables them per facility via `admin/facility-configuration` — `is_break_enabled` defaults `true` instead (the opposite polarity), preserving existing behavior rather than restricting it
