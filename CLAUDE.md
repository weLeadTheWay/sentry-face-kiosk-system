...
handleSelfServiceRecognition()
   ...
   │      [checked BEFORE the is_gs gate below - a visit already in progress is never disrupted
   │       by a facility's flag being turned off mid-visit]
   ...
   │                                                                    → 403 gatesale_not_available (Truck is unaffected - see Business Rules)
...
- **Gatesale facility eligibility (`facility_list.is_gs`, added 2026-09-03) — Truck is NOT gated by it.** A facility must have `is_gs = 1` before its kiosk will allow the Gatesale self-service process (Truck has no flag of its own and is unaffected either way — deliberately: gating both types would have silently broken every existing Truck deployment the moment this column's `false` default landed). Enforced independently, server-side, at every direct entry point (not only via the UI's recognition-time routing) via the shared `KioskController::isGatesaleEligibleFacility(KioskDevice $kiosk): bool` helper (`(bool) $kiosk->facility?->is_gs` — null-safe, so a kiosk whose facility somehow fails to resolve is treated as **not** eligible, never as an error or an open default):
...
**Error Handling:** `resolveSelfServiceDirectoryOrFail` aborts 422 on any identity mismatch. `createGatesaleVisit` aborts 403 if an active request exists at a different farm (checked again inside the locked transaction, not just at recognize time), and (added 2026-09-03) 403 if a genuinely new Gatesale visit is attempted at a facility with `is_gs = false`. Missing Host/Origin/Purpose for a genuinely new visit → 422.

**Important Files:**

| Component | File | Responsibility |
|---|---|---|
| Controller | `app/Http/Controllers/Kiosk/KioskController.php` | `gatesaleUpdateDetails`, `gatesaleCreateVisit`, `gatesaleRegisterIdentity` (takes `poses` as of 2026-09-03), `routeByIdentity`, `handleSelfServiceRecognition`, `createGatesaleVisit`, `recognize` (uses `match()` as of 2026-09-03), `isGatesaleEligibleFacility`/`gatesaleNotEligibleResponse` (the `is_gs` gate, added 2026-09-03) (same file as Kiosk Entry); `show()` eager-loads `$kiosk->load('facility')` (added 2026-09-03) so the view can read `is_gs` without a lazy-load |
| Model | `app/Models/FacilityList.php` | `is_gs` (boolean, default `false`, added 2026-09-03) — the Gatesale eligibility flag itself; see "Facility Master Data" |
...
**Configuration:** None specific; relies on the same `CACHE_STORE=database` used app-wide. `facility_list.is_gs` is per-facility data (admin-managed via `admin/facilities`), not an env var/config key.

**Known Constraints:** **`VisitorType` rows for "Gatesale" and "Truck" (and "Visitor") are not created by any seeder** (`database/seeders/` has no `VisitorTypeSeeder`) and there is **no admin CRUD controller for `VisitorType`** either (unlike `IdentityType`/`EmployeeType`, which both have full admin UIs). `NEEDS VERIFICATION`: how these rows are provisioned in a real deployment — likely manual DB insert or `tinker`, since tests create them ad hoc (`VisitorType::create(['visitor_type_name' => 'Gatesale'])`). This is a real operational gap worth flagging if asked to touch onboarding/setup. Same guided-capture tuning/no-JS-test-runner caveats noted under "Visitor Registration" apply here too, since both pages share the identical `face-enrollment.js` module. **The kiosk frontend now DOES disable the Gatesale option server-side (added 2026-09-03, same day as `is_gs` itself, after real usage surfaced that letting a visitor fill out the whole registration form only to be rejected at Save was confusing)** — see Important Files/`is_gs` UI enforcement below; this supersedes the original launch decision to leave frontend hiding optional. Backend rejection (403 `gatesale_not_available`) remains the actual, load-bearing enforcement regardless.

**Important Notes for Future Changes:** Do not scope the "one active request" check by farm or date — it is deliberately global-and-status-only. Do not change the `Cache::lock()` key to include farm — that would reopen the cross-farm race it was added to close. If you add a new self-service visitor type, it must flow through `handleSelfServiceRecognition`/`resolveSelfServiceDirectoryOrFail`, not a parallel implementation. Do not duplicate `face-enrollment.js`'s guided-capture logic inline in this file — if the kiosk registration UX ever needs to diverge from the visitor-registration one, extend the shared module's options/callbacks rather than forking it. Do not extend the `is_gs` gate to Truck without a deliberate decision — it was scoped to Gatesale only because the requirement named only Gatesale and Truck has no flag of its own; gating Truck too would need its own explicit column/decision, not a reuse of `is_gs`. If a future request asks to also gate Truck, treat it as a new flag (or a rename/generalization of `is_gs`), not silent reuse.
...
### Module: `Facility Master Data` (Phase 1: additive tables — Phase 2: core cutover — Phase 3: admin CRUD — Phase 4: Downtime Matrix/Stationary cutover — Phase 5: legacy Farm admin decommissioned — all 2026-08-27; Phase 6: `is_gs` Gatesale-eligibility flag added 2026-09-03)
...
- **Phase 6** (2026-09-03) added a single additive column, `facility_list.is_gs` (boolean, non-nullable, default `false`) — a plain per-facility attribute, **not** a new table/relationship and **not** a change to `facility_type`/`facility_category`. It controls eligibility for the Kiosk Self-Service **Gatesale** process only (Truck is unaffected — see the "Kiosk Self-Service" module's Business Rules for the full enforcement design and reasoning). Every existing facility defaulted to `false` on migration — there was no existing reliable signal (no prior "is this a Gatesale farm" flag anywhere in the schema) to seed `true` from, so this is a genuine "Gatesale starts disabled everywhere, an admin re-enables per facility" cutover. The admin `Facility` create/edit forms (Phase 3's CRUD) got a matching "Gatesale Enabled" checkbox and Data Table column in the same change, mirroring the existing `is_rtl` pattern exactly — this is the only way to set `is_gs = true` today (no separate seeder or bulk-enable tooling was added).

**Responsibilities (today):** Canonical facility-name resolution for Visitor Sync (via `FacilityResolver`); the FK target for `visitor_request.facility_id`, `kiosk_device.facility_id`, `downtime_matrix.{origin,destination}_facility_id`, and `downtime_stationary.assigned_facility_id`; full admin CRUD for facilities and facility aliases; the source of truth (`is_gs`) for Kiosk Self-Service's Gatesale eligibility gate.
...
- **`is_gs` (Phase 6) is a plain, independent boolean attribute of `facility_list` — it does not interact with `facility_type`/`facility_category`/`is_rtl` in any way and must not be inferred from them.** A facility's Gatesale eligibility says nothing about what kind of facility it is; do not assume, for example, that every `FARM` category facility should default to `is_gs = true`, or that a `PLANT`/`DC_WAREHOUSE` should default to `false` — it is purely an explicit admin choice per facility, same as `is_rtl`.

**Models:** `App\Models\FacilityType`, `App\Models\FacilityCategory`, `App\Models\FacilityList` (relations: `facilityType()`, `facilityCategory()`, `aliases()`, `kioskDevices()`, `originDowntimeMatrixRules()`, `destinationDowntimeMatrixRules()`, `downtimeStationaryRules()`; `is_gs` boolean attribute, Phase 6), `App\Models\FacilityAlias` (relation: `facility()`) — all four use the `Auditable` trait (matching `FarmList`/`FarmAlias`/`IdentityType`/`EmployeeType`'s pattern). `App\Models\VisitorRequest::facility()`, `App\Models\KioskDevice::facility()`, `App\Models\DowntimeMatrix::originFacility()`/`destinationFacility()`, and `App\Models\DowntimeStationary::assignedFacility()` (all renamed from their `*Farm()` equivalents) now belong to `FacilityList`, not `FarmList`. `FarmList`'s reciprocal `kioskDevices()`/`originDowntimeMatrixRules()`/`destinationDowntimeMatrixRules()`/`downtimeStationaryRules()` methods were removed (Phase 4) since the columns they pointed at no longer exist on those tables — moved to `FacilityList` instead.
...
27. **Gatesale self-service requires `facility_list.is_gs = true` on the kiosk's own facility — Truck is unaffected** (Kiosk Self-Service, added 2026-09-03): enforced server-side at every direct entry point (`handleSelfServiceRecognition()` for recognition-time routing, `createGatesaleVisit()` for visit creation, `gatesaleRegisterIdentity()` for new-identity registration — never only via UI hiding), and only gates *starting* a new visit/identity, never disrupts a visit already in progress if the flag is later turned off. Every facility defaults to `is_gs = false`. Do not extend this flag to gate Truck as well without a deliberate, separate decision.
...
If the request is about the Kiosk (recognition, entry/exit, Gatesale/Truck):
    ...
    facility_list.is_gs (added 2026-09-03) — Truck is NOT gated by it and must stay
    that way unless a new, deliberate decision says otherwise
    ↓
...
### Modules
Visitor Sync · Visitor Registration (guided multi-pose capture added 2026-09-03) · Kiosk Entry · Kiosk Self-Service (Gatesale/Truck, guided multi-pose capture added 2026-09-03; Gatesale facility-eligibility flag added 2026-09-03) · Face Matching (multi-pose scoring/ambiguity upgrade, Phases A–D added 2026-09-03, Phase E not done) · Google Sheets Integration · Session Auto-Resolution · Facility Master Data (live site-binding target since the 2026-08-27 cutover; `is_gs` Gatesale-eligibility flag added 2026-09-03) · Admin Management · Downtime Matrix PDF Import (Phase 1 — parse/validate/preview, added 2026-08-27; Phase 2 — Save to Production mapping into `downtime_matrix`/`downtime_stationary`, added 2026-08-28) · Authentication & Authorization · Audit Logging (cross-cutting)
